...
def load_data(url, lab_name):
    df = pd.read_csv(url)
    df['ห้องปฏิบัติการ'] = lab_name  # เพิ่มคอลัมน์เพื่อระบุว่ามาจากลิงก์ไหน (ห้องไหน)
    return df
...
try:
    ...
    if not df.empty:
        ...
        with col_m3:
            top_lab = df['ห้องปฏิบัติการ'].mode()[0] if not df.empty else "N/A"
            st.metric("ห้องที่มีผู้ใช้สูงสุด", top_lab)
        with col_m4:
            st.metric("สถานะระบบ", "Online 🟢")

        st.markdown("##")

        # --- ส่วนที่ 2: กราฟเปรียบเทียบระหว่างห้อง ---
        ...
        # --- ส่วนที่ 3: กราฟชั้นปีแยกตามห้อง ---
